...
e_step <- function(data, mu.vector, sd.vector, alpha.vector) {
  comp1.prod <- dnorm(data, mu.vector[1], sd.vector[1]) * alpha.vector[1]
  comp2.prod <- dnorm(data, mu.vector[2], sd.vector[2]) * alpha.vector[2]
...
plot_mix_comps <- function(data, mu, sigma, lam) {
  lam * dnorm(data, mu, sigma)
}

data.frame(data = wait) %>%
  ggplot() +
  geom_histogram(aes(data, ..density..), binwidth = 1, colour = "black", 
                 fill = "white") +
